...
cargo check --manifest-path=crate/async-proto/Cargo.toml --features=tokio-tungstenite026
cargo check --manifest-path=crate/async-proto/Cargo.toml --features=bytes,chrono,chrono-tz,doubloon,either,enumset,git2,gix-hash,noisy_float,rust_decimal,semver,serde_json,serenity,uuid
