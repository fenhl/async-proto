...
//!
//! * The latest release (currently [`tokio-tungstenite` 0.27](https://docs.rs/tokio-tungstenite/0.27), feature flag `tokio-tungstenite027`)
...
#[cfg(any(feature = "tokio-tungstenite021", feature = "tokio-tungstenite024", feature = "tokio-tungstenite027"))] use {
    std::{
        iter,
        mem,
    },
...
#[cfg(any(feature = "tokio-tungstenite021", feature = "tokio-tungstenite024", feature = "tokio-tungstenite027"))] const WS_MAX_MESSAGE_SIZE: usize = 16777216;
...
pub trait Protocol: Sized {
    ...
    ///
    /// The default implementation of this method is not cancellation safe.
    fn read_ws027<'a, R: Stream<Item = Result<tungstenite027::Message, tungstenite027::Error>> + Unpin + Send + 'a>(stream: &'a mut R) -> Pin<Box<dyn Future<Output = Result<Self, ReadError>> + Send + 'a>> {
        Box::pin(async move {
            let packet = stream.try_next().await.map_err(|e| ReadError {
                context: ErrorContext::DefaultImpl,
                kind: e.into(),
            ...
            match packet {
                tungstenite027::Message::Text(data) => match data.chars().next() {
                    Some('m') => {
                        let len = data[1..].parse::<usize>().map_err(|e| ReadError {
                            context: ErrorContext::DefaultImpl,
                            kind: e.into(),
                        ...
                        while buf.len() < len {
                            ...
                            if let tungstenite027::Message::Binary(data) = packet {
                                buf.extend_from_slice(&data);
                            } else {
                                return Err(ReadError {
                                    context: ErrorContext::DefaultImpl,
                                    kind: ReadErrorKind::MessageKind027(packet),
                                })
                            }
                        }
                        Self::read_sync(&mut &*buf).map_err(|ReadError { context, kind }| ReadError {
                    ...
                    _ => Err(ReadError {
                        context: ErrorContext::DefaultImpl,
                        kind: ReadErrorKind::WebSocketTextMessage027(data),
                    }),
                },
                tungstenite027::Message::Binary(data) => Self::read_sync(&mut &*data).map_err(|ReadError { context, kind }| ReadError {
                    context: ErrorContext::WebSocket {
                        source: Box::new(context),
                    },
                    kind,
    ...
    ///
    /// The default implementation of this method is not cancellation safe.
    fn write_ws027<'a, W: Sink<tungstenite027::Message, Error = tungstenite027::Error> + Unpin + Send + 'a>(&'a self, sink: &'a mut W) -> Pin<Box<dyn Future<Output = Result<(), WriteError>> + Send + 'a>>
    where Self: Sync {
        Box::pin(async move {
            let mut buf = Vec::default();
            self.write_sync(&mut buf).map_err(|WriteError { context, kind }| WriteError {
            ...
            if buf.len() <= WS_MAX_MESSAGE_SIZE {
                sink.send(tungstenite027::Message::binary(buf)).await.map_err(|e| WriteError {
                    context: ErrorContext::DefaultImpl,
                    kind: e.into(),
                })?;
            } else {
                sink.send(tungstenite027::Message::text(format!("m{}", buf.len()))).await.map_err(|e| WriteError {
                    context: ErrorContext::DefaultImpl,
                    kind: e.into(),
                })?;
                for chunk in buf.chunks(WS_MAX_MESSAGE_SIZE) {
                    sink.send(tungstenite027::Message::binary(tungstenite027::Bytes::copy_from_slice(chunk))).await.map_err(|e| WriteError {
                        context: ErrorContext::DefaultImpl,
                        kind: e.into(),
                    })?;
                }
    ...
    #[cfg(feature = "tokio-tungstenite027")]
    #[cfg_attr(docsrs, doc(cfg(feature = "tokio-tungstenite027")))]
    /// Reads a value of this type from a [`tungstenite027`] websocket.
    fn read_ws_sync027(websocket: &mut tungstenite027::WebSocket<impl Read + Write>) -> Result<Self, ReadError> {
        let packet = websocket.read().map_err(|e| ReadError {
            context: ErrorContext::DefaultImpl,
            kind: e.into(),
        })?;
    ...
    #[cfg(feature = "tokio-tungstenite027")]
    #[cfg_attr(docsrs, doc(cfg(feature = "tokio-tungstenite027")))]
    /// Writes a value of this type to a [`tungstenite027`] websocket.
    fn write_ws_sync027(&self, websocket: &mut tungstenite027::WebSocket<impl Read + Write>) -> Result<(), WriteError> {
        let mut buf = Vec::default();
        self.write_sync(&mut buf).map_err(|WriteError { context, kind }| WriteError {
            context: ErrorContext::WebSocket {
                source: Box::new(context),
        ...
        if buf.len() <= WS_MAX_MESSAGE_SIZE {
            websocket.send(tungstenite027::Message::binary(buf)).map_err(|e| WriteError {
                context: ErrorContext::DefaultImpl,
                kind: e.into(),
            })?;
        } else {
            websocket.send(tungstenite027::Message::text(format!("m{}", buf.len()))).map_err(|e| WriteError {
                context: ErrorContext::DefaultImpl,
                kind: e.into(),
            })?;
            for chunk in buf.chunks(WS_MAX_MESSAGE_SIZE) {
                websocket.send(tungstenite027::Message::binary(tungstenite027::Bytes::copy_from_slice(chunk))).map_err(|e| WriteError {
                    context: ErrorContext::DefaultImpl,
                    kind: e.into(),
                })?;
            }
    ...
    ///
    /// This can be used to get around drop glue issues that might arise with `read_ws`.
    fn read_ws_owned027<R: Stream<Item = Result<tungstenite027::Message, tungstenite027::Error>> + Unpin + Send + 'static>(mut stream: R) -> Pin<Box<dyn Future<Output = Result<(R, Self), ReadError>> + Send>> {
        Box::pin(async move {
            let value = Self::read_ws027(&mut stream).await?;
            Ok((stream, value))
        })
    }
}
...
#[cfg(feature = "tokio-tungstenite027")]
#[cfg_attr(docsrs, doc(cfg(feature = "tokio-tungstenite027")))]
pub async fn websocket027<R: Protocol, W: Protocol>(request: impl tungstenite027::client::IntoClientRequest + Unpin) -> tungstenite027::Result<(impl Sink<W, Error = WriteError>, impl Stream<Item = Result<R, ReadError>>)> {
    let (sock, _) = tokio_tungstenite027::connect_async(request).await?;
    let (sink, stream) = sock.split();
    Ok((
        sink.sink_map_err(|e| WriteError {
            context: ErrorContext::WebSocketSink,
            ...
            match msg.write_sync(&mut buf) {
                Ok(()) => Either::Left(if buf.len() <= WS_MAX_MESSAGE_SIZE {
                    Either::Left(stream::once(future::ready(tungstenite027::Message::binary(buf))))
                } else {
                    Either::Right(stream::iter(
                        iter::once(tungstenite027::Message::text(format!("m{}", buf.len())))
                        .chain(buf.chunks(WS_MAX_MESSAGE_SIZE).map(|chunk| tungstenite027::Message::binary(tungstenite027::Bytes::copy_from_slice(chunk))))
                        .collect::<Vec<_>>()
                    ))
                }.map(Ok)),
                Err(WriteError { context, kind }) => Either::Right(stream::once(future::err(WriteError {
        ...
        stream.scan(None, |state, res| {
            fn scanner<R: Protocol>(state: &mut Option<(usize, Vec<u8>)>, res: tungstenite027::Result<tungstenite027::Message>) -> Result<impl Stream<Item = Result<R, ReadError>> + use<R>, ReadError> {
                let packet = res.map_err(|e| ReadError {
                    context: ErrorContext::WebSocketStream,
                    kind: e.into(),
                })?;
                Ok(if let Some((len, buf)) = state {
                    ...
                    match packet {
                        ...
                        tungstenite027::Message::Binary(data) => Either::Right(stream::once(future::ok(R::read_sync(&mut &*data).map_err(|ReadError { context, kind }| ReadError {
                            context: ErrorContext::WebSocket {
                                source: Box::new(context),
                            },
                            kind,
